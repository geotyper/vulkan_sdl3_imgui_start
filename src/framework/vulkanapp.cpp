...
bool VulkanApp::Initialize() {
    ...
    if (!SDL_Vulkan_LoadLibrary(nullptr)) { /* nothing – just ensure library loaded */ }
    ...
    const Uint32 winFlags = SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE;
    mWindow = SDL_CreateWindow(mSettings.name.c_str(), mSettings.resolutionX,
                               mSettings.resolutionY, winFlags);
    if (!mWindow) {
        SDL_Log("SDL window creation failed: %s", SDL_GetError());
        return false;
    }

    // --- Vulkan bootstrap (instance, device, etc.) -------------------------
    if (!InitializeVulkan())              return false;
    volkLoadInstance(mInstance);
    if (!InitializeDevicesAndQueues())    return false;
    if (!InitializeSurface())             return false;
    if (!InitializeSwapchain())           return false;
    if (!InitializeFencesAndCommandPool())return false;

    vulkanhelpers::Initialize(mPhysicalDevice, mDevice, mCommandPool, mGraphicsQueue);

    if (!InitializeOffscreenImage())      return false;
    if (!InitializeCommandBuffers())      return false;
    if (!InitializeSynchronization())     return false;
...
void VulkanApp::Loop() {
    ...
    while (!mQuit) {
        ...
        while (SDL_PollEvent(&ev)) {
            switch (ev.type) {
            case SDL_EVENT_QUIT:                         mQuit = true; break;
            case SDL_EVENT_KEY_DOWN: case SDL_EVENT_KEY_UP:
                OnKey(ev.key.key,                       // key code
                      ev.key.scancode,                  // scancode
                      ev.type == SDL_EVENT_KEY_DOWN ? 1 : 0,
                      ev.key.mod);
                if (ev.key.key == SDLK_ESCAPE && ev.type == SDL_EVENT_KEY_DOWN)
                    mQuit = true;
                break;
            case SDL_EVENT_MOUSE_BUTTON_DOWN:
            case SDL_EVENT_MOUSE_BUTTON_UP:
                OnMouseButton(ev.button.button,
                              ev.type == SDL_EVENT_MOUSE_BUTTON_DOWN ? 1 : 0,
                              ev.button.clicks);
                break;
            case SDL_EVENT_MOUSE_MOTION:
                OnMouseMove(static_cast<float>(ev.motion.x),
        ...
        float  dt = static_cast<float>(t1 - t0) / static_cast<float>(freq);
        t0 = t1;
...
void VulkanApp::InitializeSettings() {
    ...
    mSettings.resolutionX = 640;
    mSettings.resolutionY = 480;
...
bool VulkanApp::InitializeVulkan()
{
    ...
    // ---- extensions provided by SDL ---------------------------------------
    Uint32 extCount = 0;
    const char* const* sdlExt = SDL_Vulkan_GetInstanceExtensions(&extCount);

    Array<const char*> extensions(sdlExt, sdlExt + extCount);
    ...
    VkResult err = vkCreateInstance(&instCI, nullptr, &mInstance);
    if (err != VK_SUCCESS) {
        CHECK_VK_ERROR(err, "vkCreateInstance");
        return false;
    }
